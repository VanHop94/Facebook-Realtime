...
public class ActionDaoImpl extends AutoWireJdbcDaoSupport implements ActionDao {
	...
	public ActionInfo queryActionOfPost(PostDetail post, String userId) {
		...
		try {
			...
			String sql = "Select * from action inner join post on action.post_id = post.id where action.post_id = ? and post.manager_id = ?";
			...
			if (rs.next()) {
				...
				long time_update = rs.getLong("time_update");
				...
				for (int i = 0; i < arr.length(); i++) {
					...
					try {
						...
						logger.error("queryActionOfPost:" + post.getPostId());
					}
				}
	...
	public boolean updateActionOfPost(ActionInfo actionInfo) {
		...
		try {
			...
			ps.setInt(13, actionInfo.getUpdate_count()+1);
